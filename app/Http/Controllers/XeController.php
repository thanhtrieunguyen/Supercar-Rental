...
class XeController extends Controller
{
    ...
    $dongxe = $request->query('dongxe');
    $hangxe = $request->query('hangxe');
    $nhienlieu = $request->query('nhienlieu');
    $min_price = $request->query('min_price');
    $max_price = $request->query('max_price');

    $xesQuery = Xe::query()->with(['hangxe', 'dongxe', 'hinhxe']);

    // Lọc theo dòng xe
    if ($dongxe) {
        $xesQuery->where('iddongxe', $dongxe);
    }

    // Lọc theo hãng xe
    if ($hangxe) {
        $xesQuery->where('idhangxe', $hangxe);
    }

    // Lọc theo nhiên liệu
    if ($nhienlieu) {
        $xesQuery->where('nhienlieu', $nhienlieu);
    }

    // Lọc theo khoảng giá
    if ($min_price) {
        $xesQuery->where('gia', '>=', $min_price);
    }

    if ($max_price) {
        $xesQuery->where('gia', '<=', $max_price);
    }

    $xes = $xesQuery->paginate(12);
    $dongXes = DongXe::all();
    $hangXes = HangXe::all();
