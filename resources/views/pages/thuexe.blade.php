...
<head>
    ...
    <style>
        ...
        .search-input:focus {
            outline: none;
            border-color: #5fcf86;
            box-shadow: 0 0 0 3px rgba(95, 207, 134, 0.15);
        }
...
<div class="search-container">
    <form action="{{ route('pages.timkiem') }}" class="search-form">
        <input class="search-input" type="search"
            placeholder="Tìm kiếm theo tên xe hoặc biển số (VD: 51G-12345)..."
            name="q" id="search" value="{{ request('q') }}" autocomplete="off">
    ...
    <div class="mt-2 text-center">
        <a href="{{ route('pages.thuexe') }}" class="clear-search">
        ...
        <ul class="product">
            ...
            <li>
                <div class="product-item">
                    @if($xe->tinhtrang == 1)
                    <div style="position: absolute; top: 50%; left: 0; right: 0; text-align: center; z-index: 10; transform: translateY(-50%);">
                        <span style="background-color: rgba(255, 82, 82, 0.85); color: white; padding: 8px 16px; border-radius: 4px; font-weight: bold;">
                            ĐÃ ĐƯỢC ĐẶT
                        </span>
                    </div>
                    @elseif($xe->tinhtrang == -1)
                    <div style="position: absolute; top: 50%; left: 0; right: 0; text-align: center; z-index: 10; transform: translateY(-50%);">
                        <span style="background-color: rgba(255, 198, 52, 0.85); color: white; padding: 8px 16px; border-radius: 4px; font-weight: bold;">
                            CHỜ XÁC NHẬN
                        </span>
                    </div>
                    @endif
                    <div class="product-top">
                        <span class="label-pos"><span class="rent"><svg width="16" height="16"
                                ...
                                @if($xe->tinhtrang == 0)
                                <p style="color: #4CAF50; font-weight: 500;">Rảnh</p>
                                @elseif($xe->tinhtrang == -1)
                                <p style="color: #FFC634; font-weight: 500;">Chờ</p>
                                @else
                                <p style="color: #FF5252; font-weight: 500;">Bận</p>
                                @endif
                            </span>
                        </span>
                        <div class="fix-img">
...
<script>
    ...
    document.getElementById('search').addEventListener('input', function(e) {
        ...
        // Check if input looks like the start of a license plate (e.g. "51G")
        if (/^\d{2}[A-Z]$/.test(value)) {
            // Suggest a complete format
            e.target.placeholder = "Đang nhập biển số xe... (Ví dụ: 51G-12345)";
        } else {
            e.target.placeholder = "Tìm kiếm theo tên xe hoặc biển số (VD: 51G-12345)...";
        }
    });
</script>
@endsection
